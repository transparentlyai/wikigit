...
export function RepositoryCard({
  ...
  const handleSync = async () => {
    try {
      ...
      toast.success(
        `Synced ${repository.owner}/${repository.name} successfully`,
      );
    } catch (error: any) {
      toast.error(
        error.message ||
          `Failed to sync ${repository.owner}/${repository.name}`,
      );
    } finally {
      setIsSyncing(false);
    }
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.error(
        error.message ||
          `Failed to remove ${repository.owner}/${repository.name}`,
      );
    }
  };
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button
            ...
            style={{
              ...
              borderRadius: "2px",
              cursor:
                isSyncing || !repository.enabled ? "not-allowed" : "pointer",
              fontSize: "0.875rem",
