...
export function SearchManager() {
  ...
  const confirmReindex = async () => {
    try {
      ...
      toast.success(
        `Search index rebuilt successfully. Indexed ${result.document_count} articles.`,
      );
    } catch (error: any) {
  ...
  return (
    <div>
      ...
      >
        ...
        <p
          style={{ marginBottom: "1rem", color: "#54595d", lineHeight: "1.6" }}
        >
          The search index is automatically updated when articles are created,
          updated, or deleted. Use this tool if you need to rebuild the entire
          index from scratch (e.g., after manual file changes or index
          corruption).
        </p>
        <button
      ...
      >
        ...
        >
          <li>
            Search indexes article titles and content for full-text search
          </li>
          <li>Title matches are boosted for higher relevance</li>
          <li>Index is automatically updated when articles are modified</li>
          <li>
            Manual reindex is only needed for troubleshooting or after direct
            file changes
          </li>
